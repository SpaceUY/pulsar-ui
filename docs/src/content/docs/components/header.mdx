...
| `variant`            | `'default' \| 'secondary'` | ❌   | `'default'`    | `secondary` shows a larger title on its own row (iOS large title style)     |
...
| `scrollY`            | `SharedValue<number>`  | ❌       | -             | When set with `variant="secondary"`, header collapses on scroll (iOS-style)   |
| `collapseThreshold`  | `number`               | ❌       | `80`          | Scroll offset (px) at which the header is fully collapsed; used with `scrollY` |
